...
@section('meta_description',
    "Read $company->reviews_count verified reviews of $company->name. Explore ratings, pricing, services, and company details on Top Firms Reviewer.")
...
@section('content')
        ...
        <div
            ...
            <div class="project-size-wrapper flex gap-2 flex-wrap">
                <button data-service="all"
                    class="tag max-[600px]:grow bg-(--secondary)/10 outline-(--secondary) outline-2 text-(--secondary) rounded-md py-1 px-2 active">All</button>
                @foreach ($reviews->unique('service_id') as $review)
                    <button data-service="{{ $review->service_id }}" {{-- <button --}}
                        class="tag max-[600px]:grow bg-(--secondary)/10 outline-(--secondary) outline-2 text-(--secondary) rounded-md py-1 px-2">
                        {{ $review->service->name }}
                    </button>
                @endforeach
    ...
    <div id="review" class="section flex flex-col gap-5">
        @foreach ($reviews as $review)
            <div
                class="reviewflex flex-col flex-1 px-4 py-2.5 gap-4 border-2 border-gray-500/20 rounded-md bg-lime-700/10 ">
                <div class="flex items-center justify-between pt-2">
                    <h1 class="text-2xl font-bold">{{ $review->project_title }} </h1>
                    <span class="flex text-lime-950 gap-2"><svg class="fill-lime-700" width="24" height="24"
                ...
                <div class="review-container py-3 border-t-(--secondary) border-t flex w-full gap-2 max-[850px]:flex-col">
                    <div class="review-project gap-5 w-[600px] max-[850px]:w-full flex flex-col">
                        <div class="flex flex-col gap-2">
                            <strong class="uppercase text-(--secondary)">The project</strong>
                            <span class="flex gap-2 items-center">
                                <svg width="24" height="24" xmlns="http://www.w3.org/2000/svg" fill-rule="evenodd"
                                    clip-rule="evenodd">
                                    <path
                            ...
                            <span class="flex gap-2 items-center">
                                ...
                                <small>{{ $review->project_size }} </small>
                            </span>
                            <span class="flex gap-2 items-center">
                                ...
                                <small>{{ $review->project_duration }} </small>
                            </span>
                        </div>
                        <div class="flex flex-col max-[850px]:hidden">

                            <strong class="uppercase text-(--secondary)">Project Summary</strong>
                            <small>
                    ...
                    <div
                        class="review-rating h-fit border-(--secondary) border-2 bg-(--secondary)/20 rounded-md flex flex-col">
                        <span class="flex flex-col max-[850px]:flex-row gap-3 items-center justify-center py-6 px-10">
                            <h5 class="text-4xl font-bold">{{ $review->rating }}</h5>
                            <div class="flex justify-center">
                                <svg width="20" height="20" class="fill-(--secondary)" clip-rule="evenodd"
                        ...
                        <div class="flex flex-col p-2">
                            <span class="flex justify-between border-t border-(--secondary) pt-3 pb-2">
                            ...
                            <span class="flex justify-between border-t border-(--secondary) pt-3 pb-2">
                                ...
                                <p>{{ $review->willing_to_refer }} </p>
                            </span>
                        </div>
                    </div>
                    <div class="review-subject flex gap-2 flex-col">
                        <strong class="uppercase text-(--secondary)">The Review</strong>
                        <p class="text-xl">"{{ $review->review }} "</p>
                        <small>{{ $review->created_at->diffForHumans() }} </small>
                        <strong class="uppercase text-(--secondary) ">Feedback Summary</strong>
                        <small class="">{{ $review->project_summary }} </small>
                    </div>
                    <div class="review-reviewer  flex flex-col">
    ...
    <div
        class="section bg-(--secondary)/40 w-full max-w-[1920px] justify-between flex flex-col gap-2 p-[50px] max-[720px]:p-5 max-[1000px]:flex-col">
