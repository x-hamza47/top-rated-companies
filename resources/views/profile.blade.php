...
@section('content')
    <div class="section mt-[150px] flex relative">
        <div class="flex gap-2 max-[840px]:flex-col">
            <img src={{ $company['logo'] }} alt="" class="w-[100px] h-[100px] object-cover rounded-md">
            <div class="name flex flex-col gap-3">
                <div class="flex flex-wrap gap-2 items-center">
                    <h1 class="text-4xl font-bold text-(--secondary)">{{ $company['name'] }}</h1>
                    @if ($company['verified'])
                        <span class="flex p-1 rounded-md gap-2 items-center bg-(--accent)/50">
                            ...
                            <h1 class="text-sm font-bold text-(--secondary)">Premier Verified</h1>
                        </span>
                    @else
                    @endif
                </div>
                <div class="flex gap-2">
                    ...
                    <div class="flex gap-4 max-[720px]:flex-col">

                        <div class="star-wrapper items-center flex gap-2">
                            <h4 class="text-(--accent) font-semibold text-xl">
                                {{ number_format($company->reviews->avg('rating'), 2) }}
                            </h4>
                            <svg clip-rule="evenodd" class="fill-(--accent)" width="20" height="20"
                            ...
                            <p class="text-(--accent)">
                                {{ $company->reviews->count() }} Reviews</p>
                        </div>
                        <span class="flex items-center flex-wrap">
                            @foreach ($company->networks->take(5) as $network)
                                <img class="w-7 rounded-full" src="{{ $network->logo }}" alt="">
                            @endforeach
                            <a href="{{ route('network', ['company' => $company->name]) }}">
                                <small class="text-(--accent) ml-2">

                                    {{ $company->networks->count() - 1 }}+ Network Connections</small></a>
                        </span>
                    </div>
                </div>
                <div class="flex gap-2 max-[720px]:flex-col">
                    <a target="_blank" href="{{ $company->detail->website }}" class="btn-secondary">
                        Visit Website
                    </a>
                    <a target="_blank" href="{{ route('profile', ['company' => $company['name']]) }}"
                        class="btn-secondary-outline">
                        Contact
                    </a>
                    <a target="_blank" href="{{ route('network', ['company' => $company->name]) }}"
                        class="btn-secondary-outline">
                        Join their Network
                    </a>
                </div>
        ...
        <button class="absolute right-10 h-fit rounded-md font-bold bg-(--secondary) p-2">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                <path class="fill-(--primary)" d="M19 24l-7-6-7 6v-24h14v24z" />
            </svg>
        </button>
    </div>
    <div class="section w-full max-w-[1920px] justify-between flex gap-2 p-[50px] max-[720px]:p-5 max-[880px]:flex-col">
        <div class="sub-section w-1/2 max-[880px]:w-full">
            <h2 class="text-3xl text-(--accent) font-bold">{{ $company->tagline }}</h2>
            <p class="text-(--black)">
                {{ $company->about }}
            </p>
            <div class="flex justify-between flex-wrap mt-10 max-[500px]:flex-col">
                <span class="flex gap-2 w-1/2 mt-8 max-[500px]:w-full relative">
                    ...
                    <p class="text-nowrap">{{ $company->detail->minProjectSize }}</p>
                </span>
                ...
                <span class="flex gap-2 relative w-1/2 mt-8 max-[500px]:w-full justify-end max-[500px]:justify-start">
                    ...
                    <p class="text-nowrap">{{ $company->detail->hourlyRate }}</p>
                </span>
                <span class="flex gap-2 relative w-1/2 mt-8 max-[500px]:w-full">
                    ...
                    <p class="text-nowrap">{{ $company->detail->employees }}</p>
                </span>
                <span class="flex gap-2 relative w-1/2 mt-8 max-[500px]:w-full justify-end max-[500px]:justify-start">
                    ...
                    <p class="text-nowrap">{{ $company->detail->locations }}</p>
                </span>
                <span class="flex gap-2 relative w-1/2 mt-8 max-[500px]:w-full">
                    ...
                    <p class="text-nowrap">{{ $company->detail->founded }}</p>
                </span>
                <span class="flex gap-2 relative w-1/2 mt-8 max-[500px]:w-full justify-end max-[500px]:justify-start">
                    ...
                    <p class="text-nowrap"> {{ $company->detail->languages }}</p>
                </span>
            </div>
        </div>
    ...
    <div class="section w-full max-w-[1920px] justify-between flex gap-2 p-[50px] max-[720px]:p-5 max-[1000px]:flex-col">
            ...
            <div class="tag-wrapper w-full flex flex-wrap gap-2">
                <div class="tag flex grow flex-col gap-2 p-3 bg-(--accent)/10 outline-2 outline-(--accent) rounded-md">
                    ...
                    <span class="flex gap-2 items-center">
                        ...
                        <p class="font-semibold text-2xl">{{ $company->detail->minProjectSize }}</p>
                    </span>
                </div>
                <div class="tag flex grow flex-col gap-2 p-3 bg-(--accent)/10 outline-2 outline-(--accent) rounded-md">
                    ...
                    <span class="flex gap-2 items-center">
                        ...
                        <p class="font-semibold text-2xl">{{ $company->detail->hourlyRate }}</p>
                    </span>
                </div>
                <div class="tag flex grow flex-col gap-2 p-3 bg-(--accent)/10 outline-2 outline-(--accent) rounded-md">
                    ...
                    <span class="flex gap-2 items-center">
                        ...
                        <p class="font-semibold text-2xl">
                            {{ number_format($company->reviews->avg('cost'), 1) }}/5
                        </p>
        ...
        <div
            ...
            <h1 class="priceRange text-xl font-bold text-(--black)" data-company="{{ $company->id }}">
        ...
        @foreach ($reviews as $review)
            <div class="review flex flex-col p-2 gap-4 outline-2 outline-(--accent) rounded-md bg-(--outline)/20 w-full">
                    ...
                    <div class="review-reviewer w-[600px] flex flex-col">
                        <div class="flex flex-col gap-3">
                            ...
                            <span class="flex gap-2 items-center">
                                ...
                                <small>{{ $review->reviewer->industry->industry }}</small>
                            </span>
                            <span class="flex gap-2 items-center">
                                <svg width="24" height="24" xmlns="http://www.w3.org/2000/svg"
                                    fill-rule="evenodd" clip-rule="evenodd">
                                ...
                                <small>
                                    {{ $review->reviewer->detail->locations }}</small>
                            </span>
                            <span class="flex gap-2 items-center">
                ...
                <button data-review="{{ $review->id }}"
                    ...
                    Full
                    Review <svg class="fill-(--primary)" xmlns="http://www.w3.org/2000/svg" width="24"
                        height="24" viewBox="0 0 24 24">
