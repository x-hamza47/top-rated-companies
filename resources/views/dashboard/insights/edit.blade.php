...
@section('content')
    <div class="dashboard-form-container pb-20">
        <div class="dashboard-form-header bg-(--color-background)">
            <h3 class="dashboard-form-title">Edit Insight</h3>
            <a href="{{ route('insights.index') }}" class="btn btn-secondary">Go Back</a>
        </div>

     <form class="flex flex-col gap-10" action="{{ route('insights.update', $insight->id) }}" method="post">
    @csrf
    @method('PUT')

    <div class="px-6 py-4 bg-(--color-background) rounded-b-2xl">
        <h2 class="sm:text-3xl text-2xl font-semibold">Edit Insight</h2>
        <div class="grid sm:grid-cols-2 gap-4 my-5 transition-all duration-500 ease-in-out ">
                ...
                <div class="inp-field w-full">
                    ...
                    <span class="relative h-11">
                        <input type="text" placeholder="Insight Title" name="title"
                            ...
                            id="title" value="{{ old('title', $insight->title) }}">
                ...
                <div class="inp-field w-full">
                    ...
                    <span class="relative h-11">
                        <select name="service_id"
                            ...
                            <option value="" class="bg-black!" disabled>Select Service</option>
            ...
            <div class="sm:col-span-2">
                <div class="inp-field w-full">
                    ...
                    <textarea name="description"
                        ...
                        placeholder="Short description">{{ old('description', $insight->description) }}</textarea>
                    @error('description')
                        <span class="error">
                            <i class="fa-solid fa-circle-exclamation error-icon"></i>
                            <p class="error-text">{{ $message }}</p>
            ...
            <div class="sm:col-span-2">
                <div class="inp-field w-full">
                    ...
                    <textarea name="article" class="summernote">{{ old('article', $insight->article) }}</textarea>
                    @error('article')
                        <span class="error">
                            <i class="fa-solid fa-circle-exclamation error-icon"></i>
                            <p class="error-text">{{ $message }}</p>
        ...
        <div class="sm:px-6 sm:py-6 px-3 py-3 border-t mt-6">
            <button type="submit"
                class="bg-(--color-secondary) cursor-pointer px-5 text-white text-center rounded-md py-2 hover:bg-(--color-primary) font-semibold">
                Update Insight
            </button>
            <a href="{{ route('insights.index') }}" class="ml-4 text-gray-500">Cancel</a>
        </div>
    </div>
</form>
    </div>
