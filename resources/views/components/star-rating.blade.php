<div class="flex gap-2 items-center  text-green-900 mt-1">
    <h4 class=" font-semibold text-base sm:text-xl">{{ round($rating, 1) }}</h4>
    {{-- ?? Stars --}}
    <div class="star-wrapper flex gap-1">
        ...
        @if ($halfStar)
            <svg clip-rule="evenodd" class="w-4.5 h-4.5 sm:w-5 sm:h-5" fill-rule="evenodd" stroke-linejoin="round"
                stroke-miterlimit="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" stroke="currentColor">
                <path
        ...
        {{-- ! Review count --}}
        @if ($reviews)
            <p class="text-xs sm:text-sm text-nowrap font-medium sm:ml-1 h-full">{{ $reviews }} Reviews</p>
        @endif
    </div>
</div>
