        ...
        <div class="flex flex-col bg-(--color-background) px-8 py-7 rounded-md">
            ...
            <form action="{{ route('auth.login') }}" method="POST" class="flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-6 mb-3">
                    ...
                    <div class="inp-field w-full h-11 relative @error('email') mb-2 @enderror">
                        <input type="text" placeholder="Email Address" name="email"
                            class="rounded-md w-full h-full border-2 border-gray-400/40 focus:border-(--color-primary) outline-none placeholder:text-gray-400 pl-4 pr-9 py-2 @error('email') invalid-input @enderror"
                            value="{{ old('email') }}">
                    ...
                    <div class="relative inp-field w-full h-11 @error('email') mb-2 @enderror">
                        <input type="password" placeholder="Password" name="password"
                            class="rounded-md w-full h-full border-2 border-gray-400/40 focus:border-(--color-primary) outline-none placeholder:text-gray-400 pl-4 pr-9 py-2 @error('password') invalid-input @enderror">
                ...
                <div class="remember-forgot flex justify-between">
                    <label><input type="checkbox" name="remember" id="" class="accent-lime-700"> Remember
                        me</label>
                    <a href="{{ route('password.request') }}" class="hover:underline font-normal text-blue-500">Forgot password?</a>
                </div>

                <button type="submit"
                    class="bg-(--color-secondary) cursor-pointer text-white w-full text-center rounded-md py-2 hover:bg-(--color-primary-hover) font-semibold ">Login</button>
